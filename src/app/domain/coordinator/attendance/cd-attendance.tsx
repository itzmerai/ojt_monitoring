...
const Attendance: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (storedCoordinatorId) {
      fetch(`http://localhost:5000/api/timesheet?coordinator_id=${storedCoordinatorId}`)
        .then((response) => {
          if (!response.ok) {
  ...
  const formatAddress = (addressData: any): string => {
    const barangay = addressData.address?.suburb || addressData.address?.neighbourhood || "";
    const municipality = addressData.address?.city || addressData.address?.town || "";
  ...
  const openMapModal = (location: string) => {
    if (location.includes(",")) {
      ...
      fetchAddress(lat, lon).then(address => setSelectedAddress(formatAddress(address))); // Format address before setting it
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row: any) => (
        ...
        >
          {row.address || row.location} {/* Display formatted address or fallback to location */}
        </span>
      ),
    },
  ];
